...
import {
  AlertTriangle, Bell, Bot, CheckCircle2, ChevronRight, Download,
  FileArchive, LoaderCircle, Mail, MessageSquare,
...
import {
  downloadAttachment, fileToStoredAttachment, submitTaskSupplement,
  type WorkflowStatus,
} from '../data/workflowStore';

const FORMAL_TYPES = new Set<EvalTask['evalType']>([
  '数据集安全评测', '深度模型可信测评', '智能体安全评测',
...
export function ResourceCenter() {
  const { user, isGuest, notifications, unreadCount, markNoticeRead: setNoticeRead, markAllNoticesRead } = useUser();
  ...
  useEffect(() => {
    ...
    const latest = user.myTasks.find(task => task.id === selectedTask.id);
    if (!latest) setSelectedTask(null);
    else if (latest !== selectedTask) setSelectedTask(latest);
  }, [user.myTasks, selectedTask?.id]);
  ...
  const filteredTasks = useMemo(() => formalTasks.filter(task => {
    ...
    const statusMatch = status === '全部状态' || task.status === status;
    return queryMatch && productMatch && statusMatch;
  }), [formalTasks, search, product, status]);

  const stats = {
    all: formalTasks.length,
    processing: formalTasks.filter(task => task.status === '处理中').length,
    supplement: formalTasks.filter(task => task.status === '待用户补充').length,
    delivered: formalTasks.filter(task => task.status === '已交付').length,
    terminated: formalTasks.filter(task => task.status === '已终止').length,
  };
  ...
  const openNotificationTask = (notice: (typeof notifications)[number]) => {
    ...
    const task = user.myTasks.find(item => item.id === notice.taskId);
    if (!task) return toast.error('该任务已不存在或当前账号无权查看');
    setSearch(''); setProduct('全部产品'); setStatus('全部状态');
    closeMessages();
  ...
  const markNoticeRead = (id: string) => {
    ...
    if (notice) openNotificationTask(notice);
  };
  ...
  const supplementMaterial = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    submitTaskSupplement(selectedTask.id, additions, user.id);
    ...
    <div className="mx-auto max-w-[1500px] px-6">
      <section className="overflow-hidden rounded-2xl border border-blue-100 bg-[linear-gradient(120deg,#eff6ff,#ffffff_48%,#eef2ff)] px-7 py-7 shadow-sm">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">{[
          ['全部任务', stats.all, 'text-slate-900'], ['处理中', stats.processing, 'text-blue-600'], ['待补充', stats.supplement, 'text-orange-600'], ['已交付', stats.delivered, 'text-emerald-600'], ['已终止', stats.terminated, 'text-slate-600'], ['未读消息', unreadCount, 'text-red-500'],
        ].map(([label, value, color]) => { const active = label === '未读消息' ? messagesOpen : status === (label === '待补充' ? '待用户补充' : label === '全部任务' ? '全部状态' : label); return <button type="button" key={String(label)} onClick={() => selectStatusCard(String(label))} className={`rounded-xl border px-4 py-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md ${active ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-100' : 'border-white bg-white/80'}`}><div className={`text-2xl font-black ${color}`}>{value}</div><div className="mt-1 text-xs text-slate-500">{label}</div></button>; })}</div>
      </section>
...
function TaskDetail({ task, onSupplement }: { task: EvalTask; onSupplement: () => void }) {
  ...
  const downloadReport = (report: NonNullable<EvalTask['reports']>[number]) => {
    if (!downloadAttachment(report)) toast.error('该文件需要连接后端文件存储后下载');
  };
  return <div className="space-y-5 pt-2">
    <div className="rounded-xl bg-slate-50 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-mono text-xs text-blue-600">{task.id}</div><h3 className="mt-2 text-lg font-black text-slate-900">{task.name}</h3><p className="mt-1 text-sm text-slate-500">{productLabel(task.evalType)}</p></div><span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold ${config.style}`}><Icon className="h-3.5 w-3.5" />{config.label}</span></div></div>
    <div className="grid gap-4 sm:grid-cols-2"><Info label="被测对象" value={task.model} /><Info label="提交方式" value={submissionLabel(task)} /><Info label="提交时间" value={task.createdAt} /><Info label="当前阶段" value={config.label} /></div>
    <div><div className="mb-2 text-sm font-bold text-slate-800">评测诉求／测试场景</div><div className="rounded-xl border bg-white p-4 text-sm leading-6 text-slate-600">{task.requirement || task.evalSet || '按提交配置开展评测'}</div></div>
    {task.publicMessage && <div className={`rounded-xl border p-4 text-sm leading-6 ${task.status === '待用户补充' ? 'border-orange-200 bg-orange-50 text-orange-800' : task.status === '已终止' ? 'border-red-100 bg-red-50 text-red-700' : 'border-blue-100 bg-blue-50 text-blue-700'}`}><MessageSquare className="mr-2 inline h-4 w-4" />{task.publicMessage}{task.supplementDueAt && <span className="ml-2 font-bold">请于 {task.supplementDueAt} 前补充</span>}</div>}
    <div><div className="mb-2 text-sm font-bold text-slate-800">已提交材料</div>{task.attachments?.length ? <div className="space-y-2">{task.attachments.map(file => <div key={file.id} className="flex items-center gap-3 rounded-lg border px-4 py-3"><FileArchive className="h-4 w-4 text-blue-500" /><span className="min-w-0 flex-1 truncate text-sm text-slate-600">{file.name}</span><span className="text-xs text-slate-400">{(file.size / 1024 / 1024).toFixed(2)} MB</span></div>)}</div> : <div className="rounded-xl border border-dashed p-4 text-sm text-slate-400">该任务使用模型 API 配置，无本地上传材料。</div>}</div>
    {task.status === '待用户补充' && <Button className="bg-orange-600 hover:bg-orange-700" onClick={onSupplement}><Upload className="mr-2 h-4 w-4" />按要求补充材料</Button>}
    <div><div className="mb-2 text-sm font-bold text-slate-800">报告与结果文件</div>{task.status === '已交付' && task.reports?.length ? <div className="space-y-2">{task.reports.map(report => <button key={report.id} onClick={() => downloadReport(report)} className="flex w-full items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-left"><ShieldCheck className="h-5 w-5 text-emerald-600" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-emerald-900">{report.name}</span><span className="text-xs text-emerald-700">管理员正式交付文件</span></span><Download className="h-4 w-4 text-emerald-600" /></button>)}</div> : <div className="rounded-xl border border-dashed p-5 text-center text-sm text-slate-400">任务交付后，报告和结果文件将在此显示。</div>}</div>
    {!!task.communications?.length && <div><div className="mb-2 text-sm font-bold text-slate-800">任务沟通记录</div><div className="space-y-2">{task.communications.slice().reverse().map(item => <div key={item.id} className="rounded-xl border px-4 py-3"><div className="flex justify-between gap-3"><b className="text-xs text-slate-700">{item.type}</b><span className="text-[10px] text-slate-400">{item.createdAt}</span></div><p className="mt-2 text-xs leading-5 text-slate-500">{item.content}</p></div>)}</div></div>}
  </div>;
}
