...
import { Activity, Bell, ChartNoAxesCombined, ChevronRight, ClipboardCheck, FileCheck2, History, KeyRound, LogOut, Mail, Menu, Search, User, Users, X } from 'lucide-react';
import { AuthBrandPanel } from '../components/AuthBrandPanel';
import { AdminOperationLogPanel, AdminWorkflowWorkbench, RegisteredUserPanel, type TaskGroup } from '../components/AdminWorkflowWorkbench';
import { WORKFLOW_EVENT, TERMINAL_WORKFLOW_STATUSES, getAdminOperationLogs, getPlatformActivities, getPlatformUsers, getWorkflowTasks, type PlatformUserRecord, type WorkflowTask } from '../data/workflowStore';
...
function Sidebar({
  ...
  return <aside className={`fixed inset-y-0 left-0 z-50 flex w-[252px] flex-col border-r border-slate-200 bg-white shadow-xl transition-transform lg:z-20 lg:translate-x-0 lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
...
function Sparkline({
  ...
  return <div className="flex h-8 items-end gap-1">{values.map((value, index) => <span key={index} className={`w-2 rounded-t ${color}`} style={{
...
function AdminOverview({
  ...
  const queue = [{
    label: '处理中',
    count: tasks.filter(task => task.status === '处理中').length,
    tone: 'text-blue-700 bg-blue-50',
    group: 'pending' as TaskGroup
  }, {
    label: '待用户补充',
    count: tasks.filter(task => task.status === '待用户补充').length,
  ...
  return <>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(card => {
        const Icon = card.icon;
        return <div key={card.label} className={`overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm`}><div className="flex items-start justify-between"><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${card.iconTone}`}><Icon className="h-5 w-5" /></span><Sparkline values={card.bars} color={card.bar} /></div><div className="mt-5"><b className="text-3xl font-black text-slate-950">{card.value}</b><h3 className="mt-1 text-sm font-bold text-slate-700">{card.label}</h3></div><p className="mt-3 text-xs font-semibold text-slate-400">{card.note}</p></div>;
      })}</div>
    <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4 border-b px-6 py-5"><div><h3 className="font-black text-slate-900">任务状态概览</h3><p className="mt-1 text-xs text-slate-400">按统一任务状态快速进入运维列表</p></div><button onClick={() => onOpenTask()} className="text-xs font-bold text-blue-600">进入任务运维 →</button></div><div className="grid divide-y sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4">{queue.map(item => <button key={item.label} onClick={() => onOpenTask(undefined, item.group)} className="flex items-center justify-between px-6 py-4 text-left transition hover:bg-slate-50"><span className="text-sm font-semibold text-slate-700">{item.label}</span><span className={`rounded-lg px-3 py-1.5 text-sm font-black ${item.tone}`}>{item.count}</span></button>)}</div></section>
...
function GlobalSearch({
  ...
  const matches = query.trim() ? {
    tasks: tasks.filter(t => `${t.id}${t.name}${t.userName}`.toLowerCase().includes(query.toLowerCase())).slice(0, 5),
    users: users.filter(u => `${u.id}${u.name}${u.contact}`.toLowerCase().includes(query.toLowerCase())).slice(0, 4)
  } : {
    tasks: [],
    users: []
  };
  return <div className="relative"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-blue-500" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="全站搜索任务 ID 或用户名" className="h-11 w-[380px] rounded-xl border border-blue-100 bg-[#f3f7fc] pl-10 pr-4 text-sm shadow-inner outline-none focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-50" />{query && <div className="absolute left-0 top-13 z-50 w-[430px] overflow-hidden rounded-2xl border bg-white shadow-2xl"><div className="bg-slate-50 px-4 py-2 text-[10px] font-bold tracking-widest text-slate-400">任务</div>{matches.tasks.map(task => <button key={task.id} onClick={() => {
        onTask(task.id);
        setQuery('');
      }} className="flex w-full justify-between px-4 py-3 text-left hover:bg-blue-50"><span><b className="block text-sm text-slate-700">{task.name}</b><small className="text-slate-400">{task.id} · {task.userName}</small></span><span className="text-xs font-bold text-blue-600">{task.status}</span></button>)}<div className="border-t bg-slate-50 px-4 py-2 text-[10px] font-bold tracking-widest text-slate-400">用户</div>{matches.users.map(user => <button key={user.id} onClick={() => {
        onUser(user.id);
        setQuery('');
      }} className="flex w-full justify-between px-4 py-3 text-left hover:bg-blue-50"><span className="text-sm font-bold text-slate-700">{user.name}</span><small className="text-slate-400">{user.id}</small></button>)}{!matches.tasks.length && !matches.users.length && <div className="py-8 text-center text-sm text-slate-400">未找到匹配结果</div>}</div>}</div>;
}
