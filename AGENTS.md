...
- 后端 DTO 的唯一类型来源是 `src/api/generated/types/**`；禁止再维护 `src/api/types.ts` 这类人工镜像，也不要为 generated DTO 再声明同义接口。
...
- 重构旧 API 时直接修改消费者并删除旧入口；禁止为了保留旧 import 路径新增 `@deprecated` re-export、兼容 barrel、类型 alias 或转发客户端。
...
- 人工复制 generated DTO 的 `src/api/types.ts` 或同义类型文件；
- 为旧 API import 路径保留 deprecated re-export、兼容 barrel、转发 client；
- 服务端列表放进 Zustand；
