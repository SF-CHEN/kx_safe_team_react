    ...
    products: [
      {
        ...
        workflow: ['进入在线体验并选择内置样例，或进入正式产品页面', '输入文本或上传受支持的文件', '执行审查并查看命中字段、上下文与风险等级', '复核结果并按建议完成遮蔽、替换或删除'],
      ...
      },
      {
        ...
        summary: '围绕质量、分布、污染、偏差与安全风险，对训练和评测数据集进行系统检查。',
        audience: '需要在模型训练、微调或上线前验证数据集质量与安全性的算法、数据与模型治理团队。',
        prepare: ['确认数据来源、授权范围与使用目的', '准备数据说明、标签定义及抽样规则', '按评测目标选择质量、分布、污染和公平性指标'],
        workflow: ['创建数据评测任务并填写基本信息', '上传授权数据集或选择演示数据', '选择指标与阈值后提交任务', '在资源中心查看进度、异常样本和整改建议'],
        outputs: ['数据质量与完整性评分', '异常、污染及重复样本清单', '分布偏差和标签一致性分析', '整改优先级建议'],
      },
      {
        id: 'aigc-moderation',
        name: 'AIGC内容审核与鉴伪',
        ...
        workflow: ['进入在线体验专区选择内容安全', '选择模态与检测能力', '选用内置正负样本或上传自有样本', '执行检测并查看标签、置信度和处置建议'],
    ...
    products: [
      {
        ...
        summary: '从鲁棒性、公平性、可解释性、隐私保护等维度评估模型可信水平。',
        audience: '需要验证深度学习模型可靠性、解释能力与风险边界的算法研发和模型治理团队。',
        prepare: ['确认模型类型、部署方式和访问边界', '准备具有代表性的测试样本', '选择与业务风险匹配的可信评测维度'],
        workflow: ['创建可信评测任务', '配置模型或接口并选择数据集', '选择评测指标后提交', '查看分维度评分、问题样本和改进建议'],
        outputs: ['可信维度评分', '薄弱项与异常样本', '对抗与鲁棒性分析', '模型改进建议'],
      },
      {
      ...
      },
      {
        ...
        workflow: ['创建智能体评测任务', '接入智能体并配置测试范围', '执行多轮对话和工具调用测试', '分析风险链路并收敛权限边界'],
      ...
      },
      {
        ...
        workflow: ['创建性能评测任务', '配置模型与评测集', '选择能力维度并执行评测', '在资源中心查看对比报告'],
      ...
      },
      {
        ...
        workflow: ['创建安全评测任务', '接入模型并选择安全测试集', '执行自动化攻击与风险测试', '查看问题样本、风险等级及整改建议'],
  ...
  },
  {
    ...
    products: [
      ...
      },
      {
        ...
        summary: '为组织提供标准研究、对标分析、体系设计与标准制定支持。',
        audience: '希望建设AI可信安全标准体系、参与标准研究或开展能力认证的组织。',
        prepare: ['明确标准对象、适用范围与建设目标', '梳理已有制度、技术能力和参考标准', '确定专家、业务和技术协同机制'],
        workflow: ['开展需求访谈和现状调研', '完成国内外标准对标与差距分析', '设计标准框架并组织评审', '支持发布、宣贯和落地验证'],
        ...
        sections: [
          {
            ...
            paragraphs: ['不能。在线体验用于少量样本和核心能力验证；需要批量数据、完整指标、正式报告、模型上传或专家交付时，应使用正式任务或预约专业服务。'],
          },
          {
            id: 'prepare',
            title: '开始前需要准备什么？',
