...
function CapabilitySection() {
  ...
  return <section id="acm-capability" className="scroll-mt-32 bg-[#f8fafc] px-6 py-20 lg:px-12"><div className="mx-auto max-w-[83%]"><SectionTitle icon={Layers3} eyebrow="核心能力" title="标识嵌入与标准检测双能力" desc="标识工具负责披露与验证，和内容风险审核、AI鉴伪形成互补能力。" /><div className="space-y-20">{bullets.map((item, index) => { const Icon = item.icon; return <ScrollReveal key={item.title}><div className={`grid items-center gap-12 lg:grid-cols-2 ${index % 2 ? 'lg:[&>*:first-child]:order-2' : ''}`}><div>{item.visual}</div><div className="relative"><span className="pointer-events-none absolute -top-14 right-3 text-8xl font-black text-slate-100">0{index + 1}</span><div className={`relative mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-black tracking-[.12em] ${index ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-blue-200 bg-blue-50 text-blue-700'}`}><Icon className="h-4 w-4" />{item.kicker}</div><h3 className="relative text-3xl font-black text-slate-900">{item.title}</h3><p className="relative mt-4 text-sm leading-8 text-slate-600">{item.desc}</p><div className="relative mt-6 grid gap-3 sm:grid-cols-2">{item.points.map(point => <div key={point} className="flex gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm font-bold text-slate-700"><CheckCircle2 className={`mt-0.5 h-4 w-4 shrink-0 ${index ? 'text-emerald-500' : 'text-blue-500'}`} />{point}</div>)}</div></div></div></ScrollReveal>; })}</div></div></section>;
}
