...
function MediaSample({ modality, marked }: { modality: Modality; marked: boolean }) {
  ...
  if (modality === 'image') return <img src={`${ASSET_ROOT}/real-image-${marked ? 'after' : 'before'}.png`} alt={marked ? '带人工智能生成合成标识的城市街景样例' : '未添加标识的城市街景样例'} className="aspect-[4/3] w-full rounded-2xl border border-slate-200 bg-slate-100 object-contain" loading="lazy" decoding="async" />;
...
export function AigcContentMarking() {
  ...
  return <div className="product-page-v3 min-h-screen bg-white">
    ...
    <section id="acm-compliance" className="scroll-mt-32 bg-[#f5f7fa] px-6 py-20 lg:px-12"><div className="mx-auto max-w-6xl"><SectionTitle icon={ShieldCheck} eyebrow="标准依据" title="对照国家标准落实标识要求" desc="先理解标准摘要和产品映射，再通过全宽原文预览核对具体条款。" tone="amber" /><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center"><div><div className="text-xs font-black tracking-widest text-blue-600">GB 45438-2025</div><h3 className="mt-2 text-2xl font-black text-slate-900">网络安全技术 人工智能生成合成内容标识方法</h3><p className="mt-3 text-sm text-slate-500">2025-02-28 发布 · 2025-09-01 实施 · 19页</p></div><div className="flex gap-3"><a href={STANDARD_PDF} target="_blank" rel="noreferrer" className="rounded-xl bg-blue-600 px-6 py-3 text-sm font-bold text-white">在线预览</a><a href={STANDARD_PDF} download className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-white px-6 py-3 text-sm font-bold text-blue-700"><Download className="h-4 w-4" />下载PDF</a></div></div></div><div className="my-6 grid gap-4 md:grid-cols-3">{[{ chapter: '第5章', title: '显式标识', desc: '规定文本、图片、音频、视频及虚拟场景的可感知标识形式、位置和可见性要求。', capability: '产品支持四模态可感知标识配置', icon: Eye }, { chapter: '第6章', title: '隐式标识', desc: '规定文件元数据应记录生成合成标签、服务提供者和内容编号等信息。', capability: '产品写入并解析关键元数据字段', icon: FileJson }, { chapter: '附录E、F', title: '格式与示例', desc: '给出文件元数据隐式标识格式与示例，为字段结构设计和结果说明提供依据。', capability: '产品提供结构校验与检测结论', icon: FileCheck2 }].map(item => { const Icon = item.icon; return <div key={item.title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-blue-50"><Icon className="h-5 w-5 text-blue-600" /></div><div><div className="text-xs font-black text-blue-600">{item.chapter}</div><div className="font-black text-slate-900">{item.title}</div></div></div><p className="mt-4 text-xs leading-6 text-slate-500">{item.desc}</p><div className="mt-4 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">能力映射：{item.capability}</div></div>; })}</div><div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div className="flex items-center gap-2 font-black text-slate-800"><FileText className="h-5 w-5 text-blue-600" />标准原文预览</div><span className="text-xs text-slate-400">GB 45438-2025</span></div><iframe title="GB 45438-2025标准预览" src={`${STANDARD_PDF}#page=1&view=FitH`} className="h-[620px] w-full bg-white" /></div><div className="mt-5 flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-5"><Info className="h-5 w-5 shrink-0 text-amber-600" /><p className="text-xs leading-6 text-amber-800">页面摘要用于帮助理解产品能力；实际合规实施仍应以标准原文、《人工智能生成合成内容标识办法》及适用要求为准。</p></div></div></section>
