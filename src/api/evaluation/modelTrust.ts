...
export async function addModelTrustEvaluationTask(
  ...
  const { data } = await client.post('/temp/model-trust-evaluation-task/add', payload, {
...
export async function updateModelTrustEvaluationTask(
  ...
  const client = createTempClient();
  const { data } = await client.put('/temp/model-trust-evaluation-task/update', payload, {
    headers: { 'Content-Type': 'application/json' },
  });
...
export async function getModelTrustEvaluationTaskById(
  ...
  const client = createTempClient();
  const { data } = await client.get('/temp/model-trust-evaluation-task/getDetailById', {
    params: { id },
  });
  return unwrapGatewayData<ModelTrustEvaluationTask>(data);
}
...
export async function pageModelTrustEvaluationTasks(
  ...
  const client = createTempClient();
  const { data } = await client.post('/temp/model-trust-evaluation-task/page', query, {
    headers: { 'Content-Type': 'application/json' },
  });
...
export async function deleteModelTrustEvaluationTask(id: number): Promise<boolean> {
  const client = createTempClient();
  const { data } = await client.delete('/temp/model-trust-evaluation-task/deleteOne', {
    params: { id },
  });
  return unwrapGatewayData<boolean>(data);
}

export async function batchDeleteModelTrustEvaluationTasks(
  ids: number[],
): Promise<boolean> {
  const client = createTempClient();
  const { data } = await client.delete('/temp/model-trust-evaluation-task/batchDel', {
    params: { ids },
